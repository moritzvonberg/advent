...
def compute_checksum(disk: list[int]):
    ...
    while front_index < back_index:
        ...
        if front_index % 2 == 0:
            ...
            while empty_space_filled < current_block_size and front_index < back_index:
                filled_from_this_block = min(
                    disk[back_index] - back_part_moved,
                    current_block_size - empty_space_filled,
                )
                empty_space_filled += filled_from_this_block
                back_part_moved += filled_from_this_block
                res += sum(
                    back_index // 2 * address
                    for address in range(offset, offset + filled_from_this_block)
                )
    ...
    res += sum(
        back_index // 2 * address
        for address in range(offset, offset + remaining_from_last_block)
    )
